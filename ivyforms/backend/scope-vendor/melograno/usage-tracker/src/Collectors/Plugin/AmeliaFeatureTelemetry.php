...
final class AmeliaFeatureTelemetry
{
    ...
    /**
     * Amelia settings / PaymentsTable gateway codes.
     *
     * @var list<string>
     */
    ...
    /**
     * Canonical telemetry keys for BI allowlist (Amelia code => payload key).
     *
     * @var array<string, string>
     */
    private const PAYMENT_TELEMETRY_KEYS = [
        'stripe' => 'stripe',
        'payPal' => 'paypal',
        'mollie' => 'mollie',
        'square' => 'square',
    ...
    public static function collect(SettingsService $settingsService, Container $container): array
    {
        ...
        $paymentGatewayUsageCounts = self::paymentGatewayTransactionCounts(
            $container,
            array_merge(self::PAYMENT_GATEWAY_CODES, ['onSite'])
        );
        ...
        foreach (self::PAYMENT_GATEWAY_CODES as $code) {
            ...
            $telemetryKey = self::PAYMENT_TELEMETRY_KEYS[$code];
            ...
            if ($featureEnabled) {
                $features[$telemetryKey] = self::paymentGatewayHasCredentials($settingsService, $code);
            }

            $featureMetrics[$telemetryKey] = [
        ...
        if ($onSiteEnabled || $onSiteUsageCount > 0) {
            if ($onSiteEnabled) {
                $features['on_site'] = $onSiteUsageCount > 0;
            }
            $featureMetrics['on_site'] = [
                'usage_count' => $onSiteUsageCount,
            ];
        }
    ...
    private static function recaptchaConfigured(SettingsService $settingsService): bool
    {
        ...
        return self::nonEmpty($recaptcha['siteKey'] ?? null)
    ...
    private static function paymentGatewayTransactionCounts(Container $container, array $gateways): array
    {
        ...
        try {
            ...
            $quotedGateways = array_map(
                static function (string $gateway): string {
            ...
            $rows = self::fetchAllRows(
                $container->getDatabaseConnection(),
                "SELECT gateway, COUNT(*) AS cnt FROM {$table} WHERE gateway IN ({$inList}) GROUP BY gateway"
            );
